...
public class EstadoRepositoryAdapter implements EstadoRepository {
    ...
    public Mono<Estado> save(Estado estado) {
        ...
        return estadoRepository.save(entity).map(estadoMapper ::toModel).as(transactionalOperator::transactional);
    }

    @Override
    public Mono<Estado> findByIdEstado(Integer idEstado) {
        return estadoRepository.findById(idEstado).map(estadoMapper::toModel).as(transactionalOperator::transactional);
    }

    @Override
    public Flux<Estado> findAll() {
        return estadoRepository.findAll().map(estadoMapper::toModel).as(transactionalOperator::transactional);
    }
}
